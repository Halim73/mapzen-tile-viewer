...
public class Utilities {
	...
	public List<Integer[]> findTiles(List<Integer> coordinates,int radius){
		List<Integer[]> tileCoords = new ArrayList<>();

		Integer[] leftCornerCoords = {(coordinates.get(0) - radius), (coordinates.get(1) - radius)};
		for(int i = 0; i < (2 * radius) + 1; i++) {
			for(int j = 0; j < (2 * radius) + 1; j++) {
				Integer[] coords = {(leftCornerCoords[0] + i), (leftCornerCoords[1] + j)};
				tileCoords.add(coords);
			}
		}
		return tileCoords;
	}
